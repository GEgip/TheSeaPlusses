...
void MyDate::showDate()const {
	cout << this->month << '/' << this->day << '/' << ((this->isAC())?this->year:-(this->year)) << ((this->isAC()) ? "  " : " BC");
 }
void MyDate::setBC() {
	if (this->year > 0)
		this->year *= -1;
...
MyDate & MyDate::operator=(const MyDate& aDate) /*servicio*/ {
	this->day = aDate.day;
	this->month = aDate.month;
	this->year = aDate.year;
...
bool MyDate::operator>(const MyDate& aDate)const {
	return ((this->year > aDate.year) || (this->year == aDate.year &&
		    this->month > aDate.month) || (this->year == aDate.year &&
			this->month == aDate.month && this->day > aDate.day));
}
bool MyDate::operator<(const MyDate& aDate) const {
